...
import { scoreSignals, buildRecommendations, generateTradesSummary, type ScoringResult } from '../../utils/recommendationEngine';
import type { Signal, Recommendation } from '../../utils/recommendationEngine';
import type { RiskFactor } from '../../utils/riskCalculations';
import { useAiTradeSummary } from '../../utils/aiTradeSummary';
...
export default function TradeRecommendations({
  ...
  return (
    <div className="trade-recs-card">
      ...
      {/* Analysis summary — rule-based instant; AI upgrade in background */}
      <TradeRecsSummary
        symbol={quote.symbol}
        result={result}
        recs={recs}
...
function TradeRecsSummary({
  ...
  const ai = useAiTradeSummary({ symbol, result, recs, fundamentals });
  const isAiReady = ai.kind === 'ready';
  const text = isAiReady ? ai.text : ruleSummary;
  return (
    <div className="trade-recs-summary">
      {isAiReady && (
        <span className="trade-recs-summary-source" title={`Generated by ${ai.modelTier}${ai.fromCache ? ' (cached)' : ''}`}>
          <span className="trade-recs-summary-source-badge">AI</span>
          {' '}{ai.modelTier}{ai.fromCache ? ' · cached' : ` · $${ai.costUsd.toFixed(4)}`}
        </span>
      )}
      <span className="trade-recs-summary-text">{text}</span>
    </div>
  );
}
